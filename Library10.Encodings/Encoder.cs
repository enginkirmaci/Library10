                ...
                finally
                {
                    ...
                    if (tryCustomEncoder && correctEncoding == null)
                    {
                        try
                        {
                            ...
                            {
                                Type encoderType = Type.GetType("Library10.Encodings." + customEncoder.Value);

                                if (encoderType != null)
                                {
                                    correctEncoding = (Encoding)System.Activator.CreateInstance(Type.GetType("Library10.Encodings." + customEncoder.Value));
                                }
                            }
                        }
                        catch (Exception)
